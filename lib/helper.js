...
async function getPackageZip() {
    // get latest version of releases from github https://github.com/YoruAkio/GTPS-Backup
    // download the zip file

    const url =
        'https://api.github.com/repos/YoruAkio/GTPS-Backup/releases/latest';

    try {
        const response = await superagent.get(url);
        const latestRelease = JSON.parse(response.text);
