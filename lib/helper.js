...
async function makeDatabaseArchive(options, destination, source) {
    ...
    const filesInBackup = fs.readdirSync(config.serverConfig.folderBackup);
    ...
    filesInBackup.forEach(file => {
        if (file === config.serverConfig.archiveName + '.rar') {
            res.data.name = file;
            res.data.size =
                getFileSize(
                    path.join(config.serverConfig.folderBackup, file),
                    'mb',
                ).toFixed(2) + ' MB';
        }
    });
