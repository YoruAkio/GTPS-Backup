...
async function checkPackageVersion() {
    ...
    if (semver.gt(packageVersionFromGithub, packageVersion)) {
        ...
        getPackageZip().then(downloadUrl => {
...
async function getPackageZip() {
    try {
        ...
        if (os.platform() === 'win32') {
            downloadUrl = `https://github.com/YoruAkio/GTPS-Backup/releases/download/${data.version}/gtps-backup-win.zip`;
        } else if (os.platform() === 'linux') {
            downloadUrl = `https://github.com/YoruAkio/GTPS-Backup/releases/download/${data.version}/gtps-backup-linux.zip`;
        }
